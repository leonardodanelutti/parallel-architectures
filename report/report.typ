...
#proof[
  ...
  - Se $not v$ non raggiunge $v$, allora nessun altro letterale $l$ raggiungibile da $v$ può raggiungere il suo complementare, se cosi fosse avremmo una catena di implicazioni $v ->^* l ->^* not l ->^* not v$ (dove $->^*$ indica la raggiungibilità). Quindi possiamo assegnare vero a $v$ e questo non porta a contraddizioni, ma questo è una contraddizione con l'ipotesi che $v$ è falsa in ogni soluzione soddisfacibile.
...
Infine, per garantire la correttezza della propagazione dei dati, l'elaborazione del grafo avviene rigorosamente livello per livello, basandosi sulle profondità estratte dall'ordinamento topologico. All'interno del kernel, i thread elaborano in parallelo i nodi appartenenti al medesimo livello e si sincronizzano esplicitamente (\_\_syncthreads()) prima di procedere al livello successivo. Questa barriera previene l'insorgenza di race condition e assicura che, nel momento in cui un nodo propaga la propria maschera di raggiungibilità verso i figli, lo stato di tutti i suoi antenati sia già stato integralmente e definitivamente risolto in una singola passata.
...
Per le euristiche basate sulla raggiungibilità (Heuristic 3 e 4), il conteggio per i nodi raggiungibili avviene propagando l'informazione nel ordine topologico inverso (dalle foglie verso le radici), dove i vertici aggregano parallelamente le maschere di bit dei propri figli, in modo simile a quanto è stato fatto per il calcolo dei backbone con la differenza che in questo caso non viene controllata la raggiungibilità verso il complementare, ma semplicemente si conta il numero di nodi raggiungibili, cioè i bit attivi nella maschera di raggiungibilità. Al fine di rispettare i limiti della VRAM, anche questa fase è strutturata a blocchi come per il caso del calcolo dei backbone.
...
La scala utilizzata è doppiamente logaritmica per poter confrontare al meglio i tempi di esecuzione, per fare in modo che i punti siano equidistanti in questa visualizzazione il numero di variabili delle istanza è stato generato con una distribuzione log-uniforme.
...
Sono stati quindi condotti benchmark su istanze più grandi per le prime tre euristiche, con un numero di variabili che va da $10^3$ a $10^6$, e con un rapporto clausole/variabili di $1.8$. 
